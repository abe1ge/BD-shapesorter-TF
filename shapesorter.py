...
'''
Instructing TensorFlow on How to Read the Data
'''
...
'''
Notice the structure of the full example in terms of its component features. 
If you were to look into the [build_image_data.py] script that we used above to write the files,
 you would see that it organises the features precisely in this arrangement. 
 You can see that we just extracted the image and label features, 
...
'''
The tf.train.shuffle_batch function is being used to get a randomly selected batch of 100 images
 from the data set. The other parameters in this function call can be adjusted for performance as described 
 [here](https://www.tensorflow.org/versions/r0.11/api_docs/python/io_ops.html#shuffle_batch).
...
'''
Now, before we start training we need to define the error, train step, 
correct prediction and accuracy (common to both models):
'''
